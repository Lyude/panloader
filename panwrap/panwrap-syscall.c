...
static LIST_HEAD(mapped_allocations);
...
void *mmap(void *addr, size_t length, int prot, int flags, int fd, off_t offset)
{
	...
	if (found) {
		struct mapped_memory *new = malloc(sizeof(new));

		new->length = length;
		new->addr = ret;
		new->gpu_va = offset;

		list_add(&new->node, &mapped_allocations);
		panwrap_log("Mapped 0x%lx@%p length=%lu\n",
			    offset, ret, length);
	} else {
		panwrap_log("Unknown memory mapping @%p: length=%lu prot = ",
			    ret, length);
